...
class EEG_Processing_User {
  ...
  int averagePeriod = 250;
  int thresholdPeriod = 1250;
  int ourChan = 3 - 1;
  float myAverage = 0.0;  
  float acceptableLimitUV = 255;
  
  int inMoov_output = 0; //value between 0-255 that is the relative position of the current uV average between the rolling lower and upper uV thresholds
  float inMoov_output_normalized = 0;
  float inMoov_output_adjusted = 0;
...
//    }  
    ...
    inMoov_output_adjusted = ((-0.1/(inMoov_output_normalized*255.0)) + 255.0);
    
    if(inMoov_output < 0) inMoov_output = 0;
    if(inMoov_output > 255) inMoov_output = 255;
 
//    println("inMoov_output: | " + (int)inMoov_output_adjusted + " |");
//    inMoov_serial.write((int)inMoov_output_adjusted); 
