...
// Define InMoove
String inMoov_portName = "/dev/tty.usbmodem1421";  //must edit this based on the name of the serial/COM port your InMoov Arduino is connected to
Serial inMoov_serial;
int inMoov_baud = 115200; //baud rate from the Arduino
InMoov inMoov;
...
void setup() {
  ...
  try {
    println("OpenBCI_GUI:  attempting to open serial port for InMoov using name = " + inMoov_portName);
    inMoov_serial = new Serial(this, inMoov_portName, inMoov_baud); //open the com port
    inMoov_serial.clear(); // clear anything in the com port's buffer
  } 
  catch (RuntimeException e) {
    println("OpenBCI_GUI: *** ERROR ***: Could not open " + inMoov_portName);
  }
}
//====================== END--OF ==========================//
//========================SETUP============================//
...
void initSystem() {
  ...
  //instantiate the InMoov object & pass the serial port along
  inMoov = new InMoov(inMoov_serial);
  eegProcessing_user = new EEG_Processing_User(nchan,openBCI.get_fs_Hz(),inMoov);
